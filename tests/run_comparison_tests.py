#!/usr/bin/env python3
"""
Standalone script to run comparison tests with detailed output.
...
def run_refactored_implementation(pspec_func, N=128, seed=13579, zics=100, H0=68):
    ...
    try:
        ...
        # Override the cosmology service with our CAMB-enabled one
        ic_generator.cosmology_service = cosmo_service
        print(f"✅ IC generator created successfully")
        
        # Step 1: Generate noise and delta field (combined like legacy)
        print(f"\n--- Step 1: Generating density field (delta) ---")
        # First generate noise
        result_noise = ic_generator.generate_noise()
        if not result_noise.get('success', False):
            print(f"❌ Noise generation failed: {result_noise.get('error', 'Unknown error')}")
            return None
        
        # Then compute density field 
        result_delta = ic_generator.compute_density_field()
        if result_delta.get('success', False):
            ...
            if delta is not None:
                ...
                print(f"❌ Delta field generation succeeded but field is None")
                return None
        else:
            print(f"❌ Delta generation failed: {result_delta.get('error', 'Unknown error')}")
            return None
        
        # Step 2: Generate LPT displacements
        print(f"\n--- Step 2: Computing LPT displacements ---")
        result_lpt = ic_generator.compute_lpt_displacements()
        
        if result_lpt.get('success', False):
            ...
            if displacements is not None and len(displacements) >= 3:
                ...
                if s1x is not None:
                    ...
                    print(f"❌ LPT computation succeeded but displacements are None")
                    return None
            else:
                print(f"❌ LPT computation succeeded but could not extract displacements")
                return None
        else:
            print(f"❌ LPT computation failed: {result_lpt.get('error', 'Unknown error')}")
            return None
