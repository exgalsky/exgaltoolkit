...
class TestMinimalExample(unittest.TestCase):
    ...
    def test_print_helpful_information_refactored(self):
        ...
        # Create simulation using new simplified API
        ...
        # Get the same power spectrum used by the legacy test
        power_spectrum = self.my_get_pspec()
        
        # Create IC generator with the CAMB power spectrum
        ...
        result = ic_generator.generate_initial_conditions(save_output=True)
        ...
        if result.get('success', False):
            ...
            positions = ic_generator.get_particle_positions()
            ...
            velocities = ic_generator.get_particle_velocities()
            if velocities is not None:
                velocities = np.asarray(velocities)
                rms_vel = np.sqrt(np.mean(velocities**2))
                print(f"Particle velocities:")
                print(f"  Shape: {velocities.shape}")
                print(f"  RMS velocity: {rms_vel:.3f} km/s")
        ...
        save_data = {}
        if delta is not None:
            save_data['delta'] = np.asarray(delta)
        if positions is not None:
            save_data['positions'] = np.asarray(positions)
        if velocities is not None:
            save_data['velocities'] = np.asarray(velocities)
        ...
        self.assertIsNotNone(ic_generator.cosmology_service)
        self.assertIsNotNone(ic_generator.grid_ops)
        self.assertIsNotNone(ic_generator.lpt_calc)
    ...
    def test_compare_legacy_vs_refactored(self):
        ...
        # Run only up to density field generation to match legacy test
        result = ic_generator.generate_initial_conditions(
            save_output=False, 
            steps=['noise', 'delta']
        )
        
        refactored_delta = ic_generator.get_density_field()
        
        if refactored_delta is not None:
            refactored_delta = np.asarray(refactored_delta)
            ...
            if np.allclose(legacy_delta, refactored_delta, rtol=1e-10, atol=1e-12):
                ...
                # Verify they have the same shape at least
                self.assertEqual(legacy_delta.shape, refactored_delta.shape)
        else:
